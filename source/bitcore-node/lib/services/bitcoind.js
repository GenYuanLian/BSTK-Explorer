...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  //console.log(txid);
  // sys.debug(tx);
  // sys.debug(self.transactionDetailedCache.toLocaleString())
  ...
  function addInputsToTx(tx, result) {
    ...
    for(var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
      ...
      console.log("addInputsToTx:------------------------------------------------------");
      console.log(input)

      if (!tx.coinbase) {
        // tx.inputSatoshis += input.valueSat;  //xd modified
        tx.inputSatoshis += input.value  * 1e8;
      }
      var script = null;
      var scriptAsm = null;
      if (input.scriptSig) {
      ...
      tx.inputs.push({
        ...
        satoshis: _.isUndefined(input.value) ? null : input.value * 1e8
      });
  ...
  function addOutputsToTx(tx, result) {
    ...
    for(var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      ...
      // xd added
      // console.log("------------------------------------------------------");
      // console.log("addOutputsToTx:")
      // console.log(out)


      // tx.outputSatoshis += out.valueSat; //xd modified
